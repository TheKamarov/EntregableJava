...
public class DigitalHouseManager {
    ...
    public void altaAlumno(String nombre, String apellido, String codigoAlumno){
    ...
    public void inscribirAlumno(List<Alumno> listaDeAlumnos, Integer codigoCurso){
        Curso cursoEncontrado = buscarCursoPorCodigo(codigoCurso);
        for (Alumno e:listaDeAlumnos) {
            if(cursoEncontrado.agregarUnAlumno(e)){
                Inscripcion unaInscripcion = new Inscripcion(e,cursoEncontrado);
                listaDeInscripciones.add(unaInscripcion);
                System.out.println("Las inscripciones se realizaron con exito");
            }else{
                System.out.println("No se pudieron inscribir porque no hay cupos");
            }
        }
    }
